...
const TRADES = ["Carpenter", "Electrician", "Plumber", "Concretor", "Plasterer", "Painter", "Steel Fixer", "Labourer", "Site Foreman", "Engineer", "Other"];
...
export default function CrewTimecards() {
  ...
  const resetForm = () => {
    setWorkerName(""); setTrade(""); setNotes("");
  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top }]}>
          ...
          <ScrollView contentContainerStyle={styles.modalContent} keyboardShouldPersistTaps="handled">
            ...
            <View style={styles.formCard}>
              ...
              <View style={[styles.formRow, { borderTopWidth: 1, borderTopColor: Colors.borderLight }]}>
                <Ionicons name="hammer-outline" size={18} color={Colors.textTertiary} style={{ marginTop: 14 }} />
                <View style={{ flex: 1 }}>
                  <Text style={styles.inputLabel}>Trade / Role</Text>
                  <Pressable onPress={() => setShowTrades(true)} style={[styles.input, { justifyContent: "center" }]}>
                    <Text style={{ color: trade ? Colors.text : Colors.textTertiary, fontSize: 15 }}>
                      {trade || "Select trade…"}
                    </Text>
                  </Pressable>
                </View>
      ...
      {/* Trade picker modal */}
      <Modal visible={showTrades} animationType="slide" presentationStyle="pageSheet" transparent>
        <Pressable style={styles.tradeOverlay} onPress={() => setShowTrades(false)} />
        <View style={[styles.tradeSheet, { paddingBottom: insets.bottom + 16 }]}>
          <View style={styles.tradeHandle} />
          <Text style={styles.tradeTitle}>Select Trade</Text>
          <FlatList
            data={TRADES}
            keyExtractor={(item) => item}
            renderItem={({ item }) => (
              <Pressable style={[styles.tradeRow, trade === item && styles.tradeRowSelected]}
                onPress={() => { setTrade(item); setShowTrades(false); }}>
                <Text style={[styles.tradeRowText, trade === item && { color: Colors.accent }]}>{item}</Text>
                {trade === item && <Ionicons name="checkmark" size={18} color={Colors.accent} />}
              </Pressable>
            )}
          />
        </View>
